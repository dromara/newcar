...
export class Translation extends AnimationBuilderItem {
  ...
  onDrawFrame(relativeFrameCount: number, parent: AnimationBuilder): void {
    this.#datas.obj!.x = this.#interpolatorx.interpolate((relativeFrameCount + 1) / this.#length);
    this.#datas.obj!.y = this.#interpolatory.interpolate((relativeFrameCount + 1) / this.#length);
  }
}
