import { createCar, config, Scene, Arc, move, Recorder, sleep, create } from "newcar";
...
car.scene = new Scene() // 初始化场景
  .add(new Arc(100, {
    ...
    .setup(async (obj) => { // 传入一个异步函数，第一个参数为此对象本身
      await sleep(100); // 等待100帧
      obj.animate(move, 30, { // 加入动画
        to: 300
      })
    }))
car.on("ready-to-play", () => { // 当CanvasKit-WASM准备充足时，播放动画
  car.play()
});
